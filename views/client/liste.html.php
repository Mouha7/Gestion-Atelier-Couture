            ...
            <div id="modal" class="z-10 fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center hide">
                <div class="bg-white rounded-lg shadow-lg w-1/3 max-h-[90vh] flex flex-col">
                    <div class="flex justify-between items-center border-b p-4">
                        <h1 class="text-lg font-semibold">Ajouter Client</h1>
            ...
            <tbody>
                ...
                <?php foreach ($array["data"] as $value) : ?>
                    ...
                    <tr>
                        ...
                        <td class="border-t-2 py-2"><?= $value["adresse"] ?></td>
                        ...
                        <td class="border-t-2 py-2">
                            ...
                            <div id="modal-<?= $counter ?>" class="z-10 fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center hide">
                                <div class="bg-white rounded-lg shadow-lg w-1/3 max-h-[90vh] flex flex-col">
                                    <div class="flex justify-between items-center border-b p-4">
                                        <h1 class="text-lg font-semibold">Modifier Client</h1>
                                    ...
                                    <div class="p-4 overflow-y-auto flex-grow">
                                        <form method="post" action="<?= WEBROOT ?>" enctype="multipart/form-data">
                                            ...
                                            <div class="mb-3">
                                                <label for="observations" class="block text-sm font-medium color-indigo">Observations</label>
                                                <textarea id="observations" name="observations" class="mt-1 p-2 border rounded w-full"><?= $value["observations"] == 'null' ? '' : $value["observations"] ?></textarea>
                                            </div>
                                            <div class="mb-3">
                            ...
                            <a href="<?= WEBROOT ?>?controller=client&action=archive-client&idUser=<?= $value["idUser"] ?>&isActif=<?= $value["isActif"] ?>" class="btn background-color-sunglow border-color-sunglow p-2 mr-1 rounded">
...
<nav class="absolute bottom-5 left-1/2 transform -translate-x-1/2 bg-white text-indigo inline-flex justify-center items-center space-x-2 rounded-lg shadow py-1 px-2">
    <a href="<?= WEBROOT ?>?controller=client&action=liste&page<?= $currentPage == 0 ? $currentPage : ($currentPage - 1) ?>" class="px-3 py-2 rounded-lg hover:bg-gray-100">&lt;</a>
    <?php for ($i = 0; $i < $array["pages"]; $i++) : ?>
        <a href="<?= WEBROOT ?>?controller=client&action=liste&page<?= $i ?>" class="px-3 py-2 rounded-lg hover:bg-gray-100 <?php if ($currentPage == $i) echo 'background-color-honeydew'; ?>"><?php echo $i + 1 ?></a>
    <?php endfor ?>
    <a href="<?= WEBROOT ?>?controller=client&action=liste&page<?= $currentPage == ($array["pages"] - 1) ? ($array["pages"] - 1) : ($currentPage + 1) ?>" class="px-3 py-2 rounded-lg hover:bg-gray-100">&gt;</a>
</nav>
